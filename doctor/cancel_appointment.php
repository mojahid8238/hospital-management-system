...
error_log("Received appointment_id: " . ($appointment_id ?? 'null'));
...
error_log("Retrieved doctor_id: " . ($doctor_id ?? 'null'));
...
if ($row = $result->fetch_assoc()) {
    if ($row['status'] === 'Cancelled') {
        $response['message'] = 'This appointment has already been cancelled.';
        echo json_encode($response);
        exit();
    }
} else {
    $response['message'] = 'Appointment not found for this doctor.';
    echo json_encode($response);
    exit();
}
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        ...
        $response['message'] = 'Appointment not found or no changes made (possibly already cancelled).';
    }
} else {
